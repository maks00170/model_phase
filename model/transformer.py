...
class CrossTransformerEncoder(nn.Module):
    ...
    def forward(self, x_z, x_p):
        B, C, Fr, T1 = x_z.shape
        pos_emb_2d_z = create_2d_sin_embedding(
            C, Fr, T1, x_z.device, self.max_period
        )  # (1, C, Fr, T1)
        pos_emb_2d_z = rearrange(pos_emb_2d_z, "b c fr t1 -> b (t1 fr) c")
        x_z = rearrange(x_z, "b c fr t1 -> b (t1 fr) c")
        x_z = self.norm_in(x_z)
        x_z = x_z + self.weight_pos_embed * pos_emb_2d_z

        
        B, C, Fr, T2 = x_p.shape
        pos_emb_2d_p = create_2d_sin_embedding(
            C, Fr, T2, x_p.device, self.max_period
        ) 
        pos_emb_2d_p = rearrange(pos_emb_2d_p, "b c fr t1 -> b (t1 fr) c")
        x_p = rearrange(x_p, "b c fr t1 -> b (t1 fr) c")
        x_p = self.norm_in(x_p)
        x_p = x_p + self.weight_pos_embed * pos_emb_2d_p

  
        for idx in range(self.num_layers):
            if idx % 2 == self.classic_parity:
                x_z = self.layers[idx](x_z)
                x_p = self.layers_t[idx](x_p)
            else:
                old_x = x_z
                x_z = self.layers[idx](x_z, x_p)
                x_p = self.layers_t[idx](x_p, old_x)

        x_z = rearrange(x_z, "b (t1 fr) c -> b c fr t1", t1=T1)
        x_p = rearrange(x_p, "b (t1 fr) c -> b c fr t1", t1=T2)
        return x_z, x_p
